...
- (instancetype)initWithFrame:(NSRect)frameRect position:(NSUInteger)position {
...
- (void)drawRect:(NSRect)dirtyRect {
    ...
    if (self.currentPosition == 1 || self.currentPosition == 2) {
        currentX = 10.0;
    } else {
        currentX = 0.0;
    }
    
    bezier = [self makeBubble:bezier withBubbleRect:NSMakeRect(currentX, 0, self.bounds.size.width - 10, self.bounds.size.height - 10) corner:8 atPostion:self.currentPosition];
